...
# file_queue = Queue()
# Dictionary to hold queues for each user
user_queues = {}
# Dictionary to track whether renaming is in progress for each user
...
@Client.on_message(filters.private & (filters.document | filters.video | filters.audio) & filters.user(ADMIN_USER_ID))
async def auto_rename_files(client, message):
    ...
    # Initialize user-specific queue if it does not exist
    if user_id not in user_queues:
        user_queues[user_id] = asyncio.Queue()
        user_renaming_operations[user_id] = {}

    # Add the file to the user's queue
    await user_queues[user_id].put((client, message, format_template, media_preference))

    # Process the user's queue if not already processing
    if not user_renaming_operations[user_id].get("is_running"):
        user_renaming_operations[user_id]["is_running"] = True
        asyncio.create_task(process_user_queue(user_id))

async def process_user_queue(user_id):
    queue = user_queues.get(user_id)
    while not queue.empty():
        client, message, format_template, media_preference = await queue.get()
        await process_file(client, message, user_id, format_template, media_preference)
        queue.task_done()
    user_renaming_operations[user_id]["is_running"] = False

async def process_file(client, message, user_id, format_template, media_preference):
    if message.document:
    ...
    # Ensure each file is processed individually
    if file_id in user_renaming_operations[user_id]:
        elapsed_time = (datetime.now() - user_renaming_operations[user_id][file_id]).seconds
        if elapsed_time < 10:
            return

    user_renaming_operations[user_id][file_id] = datetime.now()
    ...
    metadata_dir = f"Metadata/{user_id}"
    renamed_file_path = os.path.join(download_dir, renamed_file_name)
    metadata_file_path = os.path.join(metadata_dir, renamed_file_name)
    ...
    try:
        ...
        del user_renaming_operations[user_id][file_id]
    ...
    try:
        ...
        caption = c_caption.format(filename=renamed_file_name, filesize=humanbytes(message.document.file_size), duration=convert(0)) if c_caption else f"**{renamed_file_name}**"
        ...
        # Delete the upload message after completion
